  ...
  return (
    <View>
      <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-80 h-60 relative'>
        {item.addimage ? (
          <Image source={{ uri: `https://carzchoice.com/assets/backend-assets/images/${item.addimage}` }} className='size-full rounded-2xl' />
        ) : (
...
const Card = ({ item, onPress }) => {
  // let imagesArray = safeParseJSON(item.addimage, []);
